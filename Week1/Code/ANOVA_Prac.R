...
seMean <- function(logCvalue){
logCvalue <- na.omit(mammals$logCvalue) # get rid of missing values
se <- sqrt(var(logCvalue)/length(logCvalue)) # calculate the standard error
...
# Get a barplot: this function can report where the middle of the bars are on the x axis. Set the y axis limits to contain the error bars
barMids <- barplot(trophMeans, ylim=c(0, max(upperSE)),ylab="log C value (pg)")
# Add error bars using function: draws arrows between the points (x0,y0) and (x1,y1). Arrow heads at each end (code=3) and at 90 degree angles
...
# Use linear model to test whether differences are significant
