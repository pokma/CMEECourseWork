...
plot(density(genome$TotalLength, bw=5, na.rm=TRUE)) #ignore missing values
...
plot(density(Zygoptera$TotalLength), xlim=c(15, 100), ylim=c(0,0.08))
...
plot(GenomeSize ~ ForewingArea, data = genome)
...
myColours <- c('purple', 'yellow') # So choose two colours
mySymbols <- c(21,23) # And two different markers
